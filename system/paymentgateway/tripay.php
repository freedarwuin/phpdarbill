...
function tripay_validate_config()
{
    ...
    if (empty($config['tripay_secret_key'])) {
        sendTelegram("Pasarela de pago Tripay no configurada");
        r2(U . 'order/package', 'w', Lang::T("El administrador aún no ha configurado la pasarela de pago Tripay, informe al administrador"));
    }
}
...
{
    global $ui, $config;
    $ui->assign('_title', 'Tripay - Pasarela de pago');
...
function tripay_create_transaction($trx, $user)
{
    ...
    $result = json_decode(Http::postJsonData(tripay_get_server() . 'transaction/create', $json, ['Autorización: Portador ' . $config['tripay_api_key']]), true);
    if ($result['success'] != 1) {
        sendTelegram("Pago triple pago fallido\n\n" . json_encode($result, JSON_PRETTY_PRINT));
        r2(U . 'order/package', 'e', Lang::T("No se pudo crear la transacción."));
    }
    ...
    $d->save();
    r2(U . "order/view/" . $d['id'], 's', Lang::T("Crear transacción exitosa"));
